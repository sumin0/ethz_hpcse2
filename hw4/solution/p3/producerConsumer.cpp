...
int main(int argc, char* argv[])
{
    ...
    if (rankId == 0){
        factorArray = upcxx::new_array<double>(nFactors);
    }
    ...
    if (rankId == 0)
    {
        double* factorArrayLocal = factorArray.local();
        double pi_approx = 0.0;
        for (int k = 0; k < NUM_FACTORS; ++k)
        {
            pi_approx += factorArrayLocal[k];
        }
...
{
    // TODO: Compute factor
    double factor = FACTOR(factorId+1);
    ...
    upcxx::rpc_ff(0, [](int rankId, int factorId, double factor)
    {
        // Adding back the rankId worker as available again
        workers.push(rankId);
        factorArray.local()[factorId] = factor;
    }, rankId, factorId, factor);
}
...
void master(int rankCount)
{
    ...
    for (int factorId = NUM_FACTORS-1; factorId >= 0; --factorId)
    {
        ...
        int workerId = workers.front(); workers.pop();
        ...
        upcxx::rpc_ff(workerId, workerComputeFactor, workerId, factorId);
    }
